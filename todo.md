...
## Исправление размытия и выравнивания (21.01.2026 - второй раунд)

- [x] Исправить размытие фона на главной странице каталога (не работает на десктопе, работает на мобилке)
- [x] Скопировать механизм размытия со страницы контейнера на главную страницу
- [x] Убрать вертикальный сдвиг блока при открытии карточки контейнера
- [x] Выровнять расстояние header-блок на обеих страницах (должно быть идентичным)


## Исправление размытия в браузере (21.01.2026 - третий раунд)

- [x] Исправить размытие на главной странице в браузере (не работает backdrop-filter на ::before)
- [x] Разделить background-image и blur на разные слои
- [x] Поднять кнопку "Назад в каталог" чуть выше


## Финальные исправления (21.01.2026 - четвёртый раунд)

- [x] Добавить loading="lazy" к фото контейнеров для ускорения загрузки
- [x] Поднять кнопку "Назад в каталог" ещё выше
- [x] Изучить механизм размытия на странице контейнера (почему там работает)
- [x] Скопировать ТОЧНЫЙ механизм размытия со страницы контейнера на главную страницу каталога


## Исправление ошибки деплоя + blur (21.01.2026 - финальный раунд)

- [x] Исследовать почему последний checkpoint (24cf5987) падает при деплое - никаких серверных изменений, возможно глюк
- [x] Применить blur fix без поломки деплоя - добавлен background + blur 12px + !important
- [x] Поднять кнопку "Назад" ещё выше (top-0 → -top-1)
- [x] Протестировать что деплой проходит успешно - dev server работает, нет ошибок TypeScript/LSP


## ГЛУБОКОЕ СКАНИРОВАНИЕ BLUR (21.01.2026 - финальное решение)

- [ ] Просканировать ВСЕ файлы проекта на упоминания backdrop-filter, blur, catalog-page
- [ ] Найти ВСЕ места где может перезаписываться или блокироваться blur
- [ ] Проверить конфликтующие CSS правила
...
## Исправления мобильной версии (22.01.2026)
- [x] Исправить кнопку "Назад в каталог" на мобилке - залезает на заголовок контейнера
- [x] Убрать "мистическую" анимацию фона при скролле (backdrop-filter repaint issue)
- [x] Вернуть blur фона с 4px обратно на 2px

## Исправление архитектуры фона (22.01.2026)
- [x] Переделать фон - вместо CSS background использовать fixed div чтобы фон не двигался при скролле
- [x] Уменьшить blur фона (сделать четче) - возможно 1px или убрать совсем

## Замена фона на pre-blurred версию (22.01.2026)
- [x] Заменить картинку фона на уже размытую версию
- [x] Убрать backdrop-filter: blur() из FixedBackground - больше не нужен

## Исправление кнопки "Назад в каталог" на мобилке (22.01.2026)
- [x] Переместить кнопку внутрь контента (убрать absolute positioning) чтобы она не перекрывалась хедером с телефоном

## Добавление фильтра по цене (22.01.2026)
- [x] Создать раскрывающийся фильтр "Цена" между "Состояние" и "Найдено"
- [x] Добавить два input поля "от" и "до" для ручного ввода цены
- [x] Добавить range slider для визуального выбора диапазона
- [x] Автоматически определять min/max цены из текущих контейнеров в каталоге
- [x] Добавить логику фильтрации по цене в tRPC query

## Переделка фильтра по цене (22.01.2026)
- [x] Добавить range slider (двойной ползунок) для визуального выбора диапазона цен
- [x] Исправить мобильную вёрстку dropdown фильтра "Цена" - сейчас перекрывает карточки
- [x] Автоматически устанавливать min/max ползунка по самым дешёвым/дорогим контейнерам
- [x] Синхронизировать input поля с ползунком (двусторонняя связь)
- [x] Исправить инициализацию slider - показывать реальные цены вместо 0-1,000,000

## Исправление бага slider цены (23.01.2026)
- [x] Исправить max значение slider - сейчас позволяет выставить цены выше максимальной (100,000 → 226,000)
- [x] Slider должен показывать диапазон ТОЛЬКО от видимых контейнеров (после применения других фильтров)
- [x] Если выбран фильтр "Новый" → slider должен показать диапазон только новых контейнеров
- [x] Если выбран фильтр "Б/У" → slider должен показать диапазон только б/у контейнеров

## UX исправления фильтра цены (23.01.2026)
- [x] Исправить мигание карточек при движении slider - добавить debounce для запросов
- [x] Мобильная версия: кнопка цены вылезает за экран - сделать компактный вид или вынести в отдельную строку
- [x] Мобильная версия: dropdown цены уходит за край экрана - центрировать или сделать на всю ширину
- [x] Десктопная версия: учитывать размер окна для кнопки цены

## Исправление мобильного dropdown цены (23.01.2026)
- [x] Dropdown вылезает за правый край экрана - сделать на всю ширину с отступами
- [x] Кнопка фильтра показывает "₽" - изменить на "Цена" на мобильной версии

## Центрирование dropdown цены на мобильных (23.01.2026)
- [x] Dropdown вылезает за правый край экрана на мобильных устройствах
- [x] Сделать dropdown центрированным относительно экрана (не кнопки) на мобильных
- [x] Использовать fixed positioning для мобильных, absolute для десктопа

## Исправления фильтра цены - раунд 2 (26.01.2026)
- [x] На ПК-версии кнопка показывает "70 000-100 000" - непонятно, что это цены. Добавить "₽" или "Цена" для ясности
- [x] На мобильной версии dropdown открывается посередине экрана, перекрывая контейнеры. Нужно открывать между фильтрами и поиском или между поиском и "Найдено"

## Обновление логики импорта (26.01.2026)
- [x] Изменить условие импорта: теперь контейнеры импортируются, если "Доступный остаток" > 0 (ранее было только = 1)
